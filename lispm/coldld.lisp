...
(defun cold-fasload (filespec &aux qfasl-binary-file fdefine-file-pathname file-property-list)
  ...
  (with-open-file (qfasl-binary-file filespec '(:in :fixnum))
    (set-file-loaded-id filespec qfasl-binary-file)
...
(defun set-file-loaded-id (pathname stream &aux qid)
  (setq qid (vmake-pointer sym:dtp-list
  ...
  (let ((id-prop (vmake-pointer sym:dtp-list
    ...
    (store-cdr-q 'sym:property-list-area sym:cdr-nil qid)
    ;; ((nil fileversionid))
...
(defun q-fasl-storein-symbol-cell (n put-source-file-name-property)
  (prog (newp adr data sym nib)
     ...
     (cond ((atom sym)
	   ...
	   ((not (and (= n 2) (eq (pop sym) 'sym:/:property)))
...
(defun store-source-file-name-property (sym)
  (if (atom sym)
	...
	(store-cdr-q 'sym:property-list-area sym:cdr-normal fdefine-file-pathname)
	(setq sym (qintern sym))
...
(defun q-fasl-op-file-property-list ()
  (vstore-contents (+ file-property-list 2) (q-fasl-next-value))
  0)
...
(defun q-fasl-op-eval1 (&aux form pathname)
  ...
  (cond ((and (listp form) (memq (car form) '(sym:fs/:make-pathname-internal
					      sym:fs/:make-fasload-pathname)))
	 (or (setq pathname (do ((flist (cdr form) (cdr flist))
				 (plist nil)
				 (elem))
				((null flist)
				 (apply #'fs:make-pathname-internal
	...
	((and (not (atom form))
	      ...
	      (not (atom (cadr form)))
	      (eq (caadr form) 'sym:quote))
	 (store-source-file-name-property (cadadr form))
