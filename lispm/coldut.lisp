...
(defun load-parameters ()
  (load "SYS: SYS; QCOM LISP >" sym-package)
  (load "SYS: SYS; QDEFS LISP >" sym-package)
  (setq misc-function-list nil)
  (setq misc-instruction-list nil)
  (load "SYS: SYS; DEFMIC LISP >" sym-package)
	  ...
	  (t ;The number of arguments is needed anyway for the cold-load generator
	   (let ((nargs (length arglist))
	     ...
	     (cond (restarg
		    ...
		    (setq nargs (cons (- nargs 2) 77))))	;(min . max)
...
(defvar area-corresponding-arrays
	'sym:(area-name region-origin region-length region-free-pointer
	      ...
	      area-maximum-size area-swap-recommendations region-list-thread))
...
(defun create-areas (&aux high-loc)
  (do l sym:cold-load-area-sizes (cddr l) (null l)	;Area sizes in pages
  ...
  (loop with quantum = sym:page-size
	...
	as size = (* (// (+ (* (get-area-size area) sym:page-size) quantum -1) quantum)
		     quantum)
  ...
  (init-area-contents 'sym:area-swap-recommendations (vfix 0))
				...
				(fixed-p	;These usually should be scavenged, except
				 ...
				 (if (memq (car al)
					   'sym:(micro-code-symbol-area page-table-area
						 ...
						 gc-table-area address-space-map
						 region-gc-pointer region-list-thread
						 area-region-list area-region-size
						 area-maximum-size area-swap-recommendations
...
(defun init-system-communication-area (&aux (nqs 25.) adr)
...
(defun q-storage-finalize ()
  ...
  (dolist (area 'sym:(region-origin region-length region-free-pointer region-gc-pointer
		      ...
		      area-region-size area-maximum-size area-swap-recommendations
...
(defun init-address-space-map ()
  ...
  (let ((map (make-array nil 'art-8b 2000))	;Initializes to 0
    ...
    (loop for i from 0 below 400 for j from 0 by 4
	  do (vwrite (+ asm i)
		     (dpb (aref map (+ j 3)) 3010
			  (dpb (aref map (+ j 2)) 2010
			       (dpb (aref map (+ j 1)) 1010
				    (aref map j))))))))
...
(defun make-cold-1 ()
  ...
  (mapc 'cold-fasload si:cold-load-file-list)
