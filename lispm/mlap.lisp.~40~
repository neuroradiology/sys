  ...
  (PKG-BIND "COMPILER"
	...
	(LET* ((DEFAULT-CONS-AREA WORKING-STORAGE-AREA)
	       (FILENAME (FUNCALL (FS:PARSE-PATHNAME "SYS: UBIN; UCADR")
				 ':NEW-TYPE-AND-VERSION "SYM" %MICROCODE-VERSION-NUMBER))
	      )
	  (WITH-OPEN-FILE (STREAM FILENAME '(:READ))
	    (PROG (ITEM ASSEMBLER-STATE (IBASE 8) (BASE 8))
	       COM0
...
(DEFUN MA-WRITE-MCLAP-PROPS (FILENAME LIST-OF-FCTNS &OPTIONAL EXP &AUX LOSEP)
  (DOLIST (FCTN LIST-OF-FCTNS)
    (COND ((NULL (GET FCTN 'MCLAP))
	   (FORMAT T "~%~s has no MCLAP property" FCTN)
	   (SETQ LOSEP T))))
  (COND ((OR (NULL LOSEP)
