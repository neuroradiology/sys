...
(DEFMETHOD (PANE-MIXIN :SQUARE-PANE-SIZE) (MAX-WIDTH MAX-HEIGHT IGNORE IGNORE STACKING)
  (SELECTQ STACKING
    (:VERTICAL MAX-WIDTH)
    (:HORIZONTAL MAX-HEIGHT)))

(DEFMETHOD (PANE-MIXIN :SQUARE-PANE-INSIDE-SIZE) (MAX-WIDTH MAX-HEIGHT IGNORE IGNORE STACKING)
  (SELECTQ STACKING
    (:VERTICAL
...
(DEFWRAPPER (BASIC-FRAME :PRINT-NOTIFICATION) (ARGS . BODY)
  `(IF SELECTED-PANE
       (LEXPR-FUNCALL SELECTED-PANE ':PRINT-NOTIFICATION ARGS)
       . ,BODY))
...
(DEFMETHOD (BASIC-CONSTRAINT-FRAME :VERIFY-NEW-EDGES) (IGNORE IGNORE NEW-WIDTH NEW-HEIGHT)
  (COND ((BOUNDP 'INTERNAL-CONSTRAINTS)
  ...
  (COND ((LISTP DESC) DESC)
	((SYMBOLP DESC)
	 (OR (CDR (ASSQ DESC SUBSTITUTIONS))
	     (FERROR
	       NIL
	       "~A is a symbol where a list was expected (and it has no substitution)"
	       DESC)))
	(T (FERROR
	     NIL
	     "~A was found where a list (or a symbol with a substitution) was expected"
	     DESC)))))
...
ones specified by the constraint."
  ...
  (COND
    ...
    ((EQ (FIRST CON) ':FUNCALL)
     (SETQ AMOUNT (LEXPR-FUNCALL (SECOND CON) NODE AV-W AV-H W H STACKING
				 (CDDR CON))))
    ((EQ (FIRST CON) ':EVAL)
...
(DEFUN CONSTRAINT-FRAME-DRAW-A-BLANK (BLANK-TYPE NODE X Y W H SCREEN-ARRAY)
  (COND ((FUNCTIONP BLANK-TYPE)
	...
	((LISTP BLANK-TYPE)
	 (LEXPR-FUNCALL (CAR BLANK-TYPE) NODE X Y W H SCREEN-ARRAY (CDR BLANK-TYPE)))
	((ARRAYP BLANK-TYPE)
...
(DEFMETHOD (PANE-MIXIN :PANE-WIDTH) (&REST ARGS)
  (LEXPR-FUNCALL-SELF ':PANE-SIZE ARGS))
       
(DEFMETHOD (PANE-MIXIN :PANE-HEIGHT) (&REST ARGS)
  (LEXPR-FUNCALL-SELF ':PANE-SIZE ARGS))
