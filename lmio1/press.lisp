...
(DEFUN PRINT-DOVER-QUEUE ()
  ;; Avoid opening a new file job to MC if logged in to AI.
  (FS:VIEWF (IF (EQ FS:USER-LOGIN-MACHINE
		    (SI:PARSE-HOST "AI"))
		"AI: DVR: FOO; .FILE. (DIR)"
		"MC: DVR: FOO; .FILE. (DIR)")))

...
(DEFUN PRESS-START-FILE (&OPTIONAL (HOST-ADDRESS DOVER-ADDRESS))
  ...
  (SETQ PRESS-EFTP-STREAM (PRESS-OPEN-EFTP-STREAM HOST-ADDRESS DOVER-ADDRESS)))

(DEFUN PRESS-OPEN-EFTP-STREAM (HOST-ADDRESS DOVER-ADDRESS)
  (IF (NUMBERP HOST-ADDRESS)
      (IF PRESS-USE-EFTP (CHAOS:MAKE-EFTP-WRITE-STREAM HOST-ADDRESS T)
	  (LET ((CONN (CHAOS:CONNECT "AI-CHAOS-11" "DOVER")))
	    (AND (STRINGP CONN)
		 (FERROR NIL
			 "~A - cannot connect to DOVER server at AI-CHAOS-11" CONN))
	    (CHAOS:STREAM CONN)))
...
(DEFUN PRESS-CHAR (CHAR)
  (COND ((< CHAR 200)				;Printing
	...
	((= CHAR #\CR)
	 (LET ((Y (- PRESS-Y (OR PRESS-INTERLINE-SPACING (SIXTH PRESS-CURRENT-FONT)))))
...
(DEFUN PRESS-LOOKUP-FONT (FAMILY FACE POINT-SIZE ROTATION)
  (DO ((L PRESS-FONT-LIST (CDR L))
       (I 0 (1+ I)))
      ((NULL L))
    (AND (EQUAL FAMILY (CAAR L))
	 (EQUAL FACE (SECOND (CAR L)))
...
(DEFUN PRINT-PRESS-FILE (FILE-NAME &REST OPTIONS)
  (WITH-OPEN-FILE (INPUT-STREAM FILE-NAME)
    (WITH-OPEN-STREAM (OUTPUT-STREAM (BIND-PRESS-VARIABLES
				       (MULTIPLE-VALUE-BIND (NIL NIL NIL NIL NIL
							     HOST-ADDRESS)
					   (PRESS-DECODE-OPTIONS OPTIONS)
					 (PRESS-OPEN-EFTP-STREAM HOST-ADDRESS))))
...
(DEFUN PRINT-FILE (FILE-NAME &REST OPTIONS)
  (WITH-OPEN-FILE (STREAM FILE-NAME '(:READ))
    (LEXPR-FUNCALL #'PRINT-FROM-STREAM
		   STREAM (FUNCALL STREAM ':TRUENAME) OPTIONS)))

(DEFUN SPOOL-FILE (FILE-NAME &REST OPTIONS)
  (WITH-OPEN-FILE (STREAM FILE-NAME '(:READ))
    (LEXPR-FUNCALL #'PRINT-FROM-STREAM
		   STREAM (FUNCALL STREAM ':TRUENAME) ':SPOOL T OPTIONS)))

(DEFUN PRINT-FROM-STREAM (INPUT-STREAM FILE-NAME &REST OPTIONS
			  &AUX CREATION-DATE FONT-NAME FACE-NAME FONT-SIZE PAGE-HEADINGS
			       N-COPIES HOST-ADDRESS)
  (DECLARE (SPECIAL FILE-NAME CREATION-DATE))
  (BIND-PRESS-VARIABLES
    (MULTIPLE-VALUE (FONT-NAME FACE-NAME FONT-SIZE PAGE-HEADINGS N-COPIES HOST-ADDRESS)
      (PRESS-DECODE-OPTIONS OPTIONS))
    ...
    (UNWIND-PROTECT
      (PROGN (PRESS-START-FILE HOST-ADDRESS)
	     ...
	     (PRESS-SELECT-FONT (IF PRESS-INTERPRET-XGP-ESCAPE 0
				    (PRESS-DEFINE-FONT FONT-NAME FACE-NAME FONT-SIZE 0)))
	     (DO ((CH))
...
(DEFUN PRESS-DECODE-OPTIONS (OPTIONS &AUX (FONT-NAME "LPT")
			     (FACE-NAME "")
			     (FONT-SIZE 8)
			     (PAGE-HEADINGS T)
			     (N-COPIES 1)
			     (HOST-ADDRESS DOVER-ADDRESS))
  ;; Special variables are just set rather than returned
  (DECLARE (RETURN-LIST FONT-NAME FACE-NAME FONT-SIZE PAGE-HEADINGS N-COPIES HOST-ADDRESS))
  (DO ((O OPTIONS (CDDR O)))
      ((NULL O))
    (SELECTQ (CAR O)
      (:FONT-NAME (SETQ FONT-NAME (CADR O)))
      (:FACE-NAME (SETQ FACE-NAME (CADR O)))
      (:FONT-SIZE (SETQ FONT-SIZE (CADR O)))
      (:FONT (MULTIPLE-VALUE (FONT-NAME FACE-NAME FONT-SIZE)
	       (DECODE-FONT-NAME (CADR O))))
      (:PAGE-HEADINGS (SETQ PAGE-HEADINGS (CADR O)))
      (:COPIES (SETQ N-COPIES (CADR O)))
      (:HOST-ADDRESS (SETQ PRESS-USE-EFTP T
			   HOST-ADDRESS (CADR O)))
      (:FILE (SETQ HOST-ADDRESS (CADR O)
		   PRESS-USE-EFTP NIL))
      (:SPOOL (SETQ HOST-ADDRESS (FORMAT NIL "MC: .DOVR.; ~A >" USER-ID)
		    PRESS-USE-EFTP NIL))
      (:EFTP (SETQ HOST-ADDRESS DOVER-ADDRESS
		   PRESS-USE-EFTP T))
      (:XGP (SETQ PRESS-INTERPRET-XGP-ESCAPE T
		  PAGE-HEADINGS NIL))
      (OTHERWISE (FERROR NIL "~S is an unknown keyword" (CAR O)))))
  (VALUES FONT-NAME FACE-NAME FONT-SIZE PAGE-HEADINGS N-COPIES HOST-ADDRESS))
  ...
  (WITH-OPEN-FILE (STREAM FILE-NAME '(:READ :RAW :SUPER-IMAGE))
    (LEXPR-FUNCALL #'PRINT-FROM-STREAM
		   (LET-CLOSED ((FILE-STREAM STREAM))
		     #'XGP-FILE-STREAM)
		   (FUNCALL STREAM ':TRUENAME) ':XGP T OPTIONS)))
...
(DEFVAR DIRED-PRINT-OPTIONS NIL)

(DEFUN (:DOVER :DIRED-PRINT-FUNCTION) (PATHNAME TYPE)
  (SELECTQ TYPE
    (:SUDS-PLOT
     (AND (NOT (FBOUNDP ':DPLT-PRINT-FILE))
	  (LOAD "SYS: IO1; DPLT PKG >")
	  (PKG-LOAD 'DPLT '(:NOCONFIRM)))
     (:DPLT-PRINT-FILE PATHNAME)
     T)
    (:XGP
     (LEXPR-FUNCALL #'PRINT-XGP-FILE PATHNAME DIRED-PRINT-OPTIONS))
