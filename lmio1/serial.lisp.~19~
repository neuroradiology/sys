...
;Make a stream.  Allowed keywords are:
...
; also a variety of more random options, see the :PUT message
...
;Print the complete hardware status
(DEFUN SERIAL-STATUS ()
...

(DEFFLAVOR BASIC-SERIAL-STREAM (INPUT-UNIBUS-CHANNEL OUTPUT-UNIBUS-CHANNEL
				(UART-COMMAND 5)	;receive, transmit enable
				(UART-MODES 0) (FORCE-OUTPUT T)
				(DLE-CHARACTER 0) (SYN1-CHARACTER 0) (SYN2-CHARACTER 0)
				(SERIAL-UNRCHF NIL) (SERIAL-ERROR-MASK 0))
	   (SI:BIDIRECTIONAL-STREAM SI:CHARACTER-STREAM
	    SI:UNBUFFERED-LINE-INPUT-STREAM SI:LINE-OUTPUT-STREAM-MIXIN
  ...
  (:DOCUMENTATION :BASE-FLAVOR "Basis for RS232 Serial I//O Streams"))

(DEFFLAVOR SERIAL-STREAM () (BASIC-SERIAL-STREAM SI:BUFFERED-OUTPUT-STREAM)
...
(DEFMETHOD (BASIC-SERIAL-STREAM :INIT) (INIT-PLIST)
  ...
  (LET ((REQUEST-TO-SEND T)		;Parse options
    ...
    (FUNCALL-SELF ':PUT ':SYNCHRONOUS-MODE SYNCHRONOUS-MODE)
    (FUNCALL-SELF ':PUT ':REQUEST-TO-SEND REQUEST-TO-SEND)
    (FUNCALL-SELF ':PUT ':DATA-TERMINAL-READY DATA-TERMINAL-READY)
    ...
    (FUNCALL-SELF ':PUT ':NUMBER-OF-STOP-BITS 1)
    (FUNCALL-SELF ':PUT ':PARITY ':EVEN)
    (FUNCALL-SELF ':PUT ':NUMBER-OF-DATA-BITS 7)
    (FUNCALL-SELF ':PUT ':BAUD 300.)
    ...
    (LOOP FOR (KWD VAL) ON (CDR INIT-PLIST) BY 'CDDR
	  UNLESS (MEMQ KWD '(:REQUEST-TO-SEND :DATA-TERMINAL-READY :SYNCHRONOUS-MODE
	    ...
	    DO (FUNCALL-SELF ':PUT KWD VAL))
    (FUNCALL-SELF ':RESET)))	;Sets up the Unibus channels
...
(DEFMETHOD (BASIC-SERIAL-STREAM :GET) (PROP)
  (LET ((MODE (SERIAL-READ-MODE))
	(STATUS (%UNIBUS-READ 764162))
	(COMMAND (%UNIBUS-READ 764166))
	(SYNCHRONOUS-P NIL))
...
(DEFMETHOD (BASIC-SERIAL-STREAM :PUT) (PROP VAL)
  (LET ((MODE (SERIAL-READ-MODE))
	(SYNCHRONOUS-P NIL))
    (AND (ZEROP (LDB 1002 MODE)) (SETQ SYNCHRONOUS-P T))
    (SELECTQ PROP
      (:REQUEST-TO-SEND (SERIAL-WRITE-COMMAND (LOGIOR 40 UART-COMMAND)))
      (:DATA-TERMINAL-READY (SERIAL-WRITE-COMMAND (LOGIOR 2 UART-COMMAND)))
      (:NUMBER-OF-STOP-BITS
...
(DECLARE-FLAVOR-INSTANCE-VARIABLES (BASIC-SERIAL-STREAM)
(DEFUN SERIAL-WRITE-COMMAND (CMD)
  (SETQ UART-COMMAND CMD)			;remember it
  (IF (BOUNDP 'OUTPUT-UNIBUS-CHANNEL)
...
(DEFMETHOD (BASIC-SERIAL-STREAM :CLOSE) (&OPTIONAL ABORT-P)
  (AND (NOT ABORT-P) (BOUNDP 'OUTPUT-UNIBUS-CHANNEL) OUTPUT-UNIBUS-CHANNEL
       (FUNCALL-SELF ':FINISH))
  (%UNIBUS-WRITE 764112 (DPB 0 0701 (%UNIBUS-READ 764112)))	;Turn off interrupt
  ;Following code commented out, due to bug in MAKUNBOUND (doesn't work on instance vars)
;  (IF (BOUNDP 'INPUT-UNIBUS-CHANNEL)
;      (RETURN-UNIBUS-CHANNEL (PROG1 INPUT-UNIBUS-CHANNEL
;				    (MAKUNBOUND 'INPUT-UNIBUS-CHANNEL))))
;  (IF (BOUNDP 'OUTPUT-UNIBUS-CHANNEL)
;      (RETURN-UNIBUS-CHANNEL (PROG1 OUTPUT-UNIBUS-CHANNEL
;				    (MAKUNBOUND 'OUTPUT-UNIBUS-CHANNEL))))
  (AND (BOUNDP 'INPUT-UNIBUS-CHANNEL) INPUT-UNIBUS-CHANNEL
       (RETURN-UNIBUS-CHANNEL (PROG1 INPUT-UNIBUS-CHANNEL
				     (SETQ INPUT-UNIBUS-CHANNEL NIL))))
  (AND (BOUNDP 'OUTPUT-UNIBUS-CHANNEL) OUTPUT-UNIBUS-CHANNEL
       (RETURN-UNIBUS-CHANNEL (PROG1 OUTPUT-UNIBUS-CHANNEL
				     (SETQ OUTPUT-UNIBUS-CHANNEL NIL)))))

(DEFMETHOD (BASIC-SERIAL-STREAM :FINISH) ()
...
(DEFMETHOD (BASIC-SERIAL-STREAM :CLEAR-INPUT) ()
  (FUNCALL-SELF ':RESET NIL))

(DEFMETHOD (BASIC-SERIAL-STREAM :RESET) (&OPTIONAL (ABORT-OUTPUT ':ABORT))
    (FUNCALL-SELF ':CLOSE ABORT-OUTPUT)	;Get rid of unibus channels if any
    ...
    (SETQ INPUT-UNIBUS-CHANNEL (GET-UNIBUS-CHANNEL 264 764162 2 764160 2))
    (SETQ OUTPUT-UNIBUS-CHANNEL (GET-UNIBUS-CHANNEL 264 764162 1 764160 1 
						    764166 (LOGAND UART-COMMAND 376)))
    (%UNIBUS-WRITE 764112 (DPB 1 0701 (%UNIBUS-READ 764112)))) ;Turn on interrupt

;;; Stream Input Protocol
(DEFMETHOD (BASIC-SERIAL-STREAM :TYI) (&OPTIONAL IGNORE)
...
(DEFMETHOD (BASIC-SERIAL-STREAM :SEND-OUTPUT-BUFFER) (IGNORE NEW-INDEX)
  (UNIBUS-CHANNEL-ADVANCE OUTPUT-UNIBUS-CHANNEL NEW-INDEX)
  (%UNIBUS-WRITE 764166 UART-COMMAND))	;Enables the transmitter

(DEFMETHOD (BASIC-SERIAL-STREAM :DISCARD-OUTPUT-BUFFER) (IGNORE)
  NIL)
...
(DEFMETHOD (BASIC-SERIAL-STREAM :FRESH-LINE) ()
  (FUNCALL-SELF ':TYO #\CR))
...
(DEFMETHOD (BASIC-SERIAL-STREAM :AFTER :TYO) (IGNORE)
  (AND FORCE-OUTPUT (FUNCALL-SELF ':FORCE-OUTPUT)))

(DEFMETHOD (BASIC-SERIAL-STREAM :AFTER :STRING-OUT) (&REST IGNORE)
  (AND FORCE-OUTPUT (FUNCALL-SELF ':FORCE-OUTPUT)))

(DEFFLAVOR SERIAL-XON-XOFF-MIXIN ((BUFFERING-CAPACITY 10.)) ()
  (:INITABLE-INSTANCE-VARIABLES BUFFERING-CAPACITY)
  (:REQUIRED-FLAVORS BASIC-SERIAL-STREAM)
